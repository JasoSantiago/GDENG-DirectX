...
void SceneReader::readFromFile()
{
	...
	while (std::getline(sceneFile, readLine)) {
		if (index == 0) {
			...
			int rb = std::stoi(readLine);
			index = 0;
			Cylinder* cylinder;
			switch (objectType)
			{
			case AGameObject::PrimitiveType::CUBE:
				GameObjectManager::getInstance()->addGameObject(new Cube(objectName, position, scale, Vector3D(), rotation, rb));
				break;
			...
			case AGameObject::PrimitiveType::SPHERE:
				GameObjectManager::getInstance()->addGameObject(new Sphere(objectName, position, scale.x, 5, rb));
				break;
			case AGameObject::PrimitiveType::CYLINDER:
				cylinder = new Cylinder("Cylinder", 3.0, 1.0, 32,rb, position, scale, rotation);
				GameObjectManager::getInstance()->addGameObject(cylinder);
				break;
			case AGameObject::PrimitiveType::CAPSULE:
				//cylinder = new Cylinder("Cylinder", 3.0, 1.0, 32, rb, position, scale, rotation);
				GameObjectManager::getInstance()->addGameObject(new LoadedMeshObject("Capsule", position, scale, rotation, L"Assets\\Meshes\\capsule.obj"));
				break;
			default:
				break;
			}
